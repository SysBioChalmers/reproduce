...
1. Create a standard curve abundances Vs. iBAQ values of ES (L fraction in the iBAQ data)
...
4. Use this value for rescaling the IS abundances in iBAQ: `IS abundance = iBAQ.H value * (average total IS abundance)/(sum of all iBAQ.H values)`
...
scaling <- mean_totProt/colSums(iBAQdata[,iBAQpos], na.rm = TRUE)
plotAllES(ESdata,'iBAQ.L.T4h_',scaling,FALSE)
```

<center>**Figure 5: ES curves (abundance Vs iBAQ values of UPS2), original (red) Vs rescaled (green)**</center>
<br>
...
<center>**Figure 9: ES curves for all 6 measurements of UPS2, based on L intensities**</center>
<br>

The fits are actually better than in figure 3, suggesting that, at least for this dataset, protein abundances correlate better to MS intensities than iBAQ values. We will then proceed with only MS intensities in this section.
<br>
...
```{r fig.align="center"}
mean_rescaled2 <- plotTotalProt(iBAQdata,'Abundance2RescaledIS.')
```
...
<center>**Figure 12: ES curves (abundance Vs H intensities of UPS2), original (red) Vs rescaled (green)**</center>
<br>
...
<br>
<center>**Figure 13: ES curves (abundance Vs H intensities of UPS2), original (red) Vs rescaled (green). Average coefficient of variation (CVm) within each protein in the log space is shown in the upper left corner.**</center>
<br>
...
```{r}
SILACdata <- getSampleAbundance(SILACdata,iBAQdata,'Abundance2Rescaled')
```

```{r fig.align="center"}
mean_sampleRescaled2 <- plotTotalProt(SILACdata,'Abundance2Rescaled.R..1_')
```
...
abundance <- SILACdata[,grep('Abundance2Rescaled.R',names(SILACdata))]
plotVariability(abundance,c('.R1.1','.R2.1','.R3.1'),'Biological Variability')
plotVariability(abundance,c('_Batch1','_Batch2','_Batch3'),'Technical Variability')
plotPCA(abundance,'PCA')
```

<center>**Figure 15: Comparison of data variability without rescaling (top row), rescaling to iBAQ values (middle row) and rescaling to MS intensities (bottom row). In the variability plots (left and middle columns) green is a FC < 2, yellow between 2 and 10, and red > 10. In the PCA plots (right column), colors refer to technical replicates and shapes to biological replicates.**</center>
<br>
