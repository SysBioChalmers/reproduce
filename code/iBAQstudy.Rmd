...
Here we will go through the typical way of deducing protein abundances [fmol/sample] from SILAC/iBAQ data (sections 1-2), and compare it to rescaling values to a fix total protein abundance based on MS intensities, to asses the usefulness of the external standard curve (section 3). The main observation that come from this is as MS measurements are so variable, it's impossible to find a unique ES curve, hence normalizing to a fix total protein abundance is as good as using the "optimal" fit from the ES curve.
<br>
...
For all MS intensitiy values in both iBAQ and SILAC, we need to normalize by the number of theoretical peptides for each protein:
...
The alternative: All normalized MS intensities of the H fraction added up together should be proportional to the average total detection from figure 1.

1. Create a standard curve abundances (fmol/sample) Vs. MS intensities of ES (L fraction in the iBAQ data)
2. Use the standard curve to infer abundances of IS in the iBAQ data (from the H fraction of the MS intensities)
3. Compute the average total detected protein amount of IS (pg/sample)
4. Use this value for computing the IS abundances (fmol/sample) in iBAQ: `IS abundance = H intensity * (average total IS abundance)/(sum of all H intensities*MW values)`
...
2. Rescale to fixed value = 6 ug/sample (first we need total protein content)
3. Is the order in ES data always the same?
4. show only technical variability in abundance plots
5. Validation to stoichiometry of ribosomal proteins
6. ES curves relax slope = 1?
