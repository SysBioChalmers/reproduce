...
Now, let's see how are the predictions of ribosomal subunits stoichiometry in the 6 measurements of the internal standard (as they should all be the same, being that they are all belonging to the same sample). For that first we need to create dataframes with only ribosomal proteins, and then plot for each method the corresponding data:
...
So in terms of accuracy, methods 1, 2 and 4 perform similarly well, and method 3 performs significantly worse.
