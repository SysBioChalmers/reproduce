...
# add the theoretical number of peptides column to the SILAC data as well:
SILACdata <- merge(ISdata[,1:2], SILACdata, by = 'Protein.IDs', all.x = FALSE, all.y = TRUE)
```
...
SILACdata <- rescaleData(SILACdata,'Abundance.iBAQ.R','iBAQrescaled.R',6e6)    #6 ug
```
...
SILACdata <- rescaleData(SILACdata,'Intensity.L.R','TPA.R',6e6)     #6 ug
```
...
SILACdata <- normalizeIntensities(SILACdata,'theo.peptides')
```
...
SILACdata <- rescaleData(SILACdata,'normInt.Ntheo.L.R','TPAnorm.R',6e6)     #6 ug
```
...
```{r accuracy-ribosome-is, fig.height = 7, fig.width = 6, fig.align="center", fig.cap = "\\label{fig:accuracy-ribosome}Comparison of predicted ribosomal subunit abundances [fmol/sample] in the internal standard, according to all methods. Colors correspond to different technical replicates. The median value is displayed with a segmented line, and the median fold change to that line for all data is displayed."}
