...
As iBAQ values don't add up always to the same totals (Figure S6), we should asses the benefits of rescaling all of these values to add up to the injected amounts:
...
```{r}
ESdata    <- rescaleData(ESdata,'Intensity.L.T4h_','TPA.ES.',1.1e6) #1.1 ug = 1.1*10^6 pg
ISdata    <- rescaleData(ISdata,'Intensity.H.T4h_','TPA.IS.',6e6)   #6 ug = 6*10^6 pg
...
```{r}
ESdata    <- rescaleData(ESdata,'Intensity.L.T4h_','TPAnorm.ES.',1.1e6) #1.1 ug = 1.1*10^6 pg
ISdata    <- rescaleData(ISdata,'Intensity.H.T4h_','TPAnorm.IS.',6e6)   #6 ug = 6*10^6 pg
...
```{r general-comp, fig.height = 6, fig.width = 10, fig.align = "center", fig.cap = "Figure S1: Comparison of predictions [fmol/sample] between all methods. Green is a FC < 2, yellow between 2 and 10, and red > 10."}
...
plotVariability(TPAvsIBAQ, c('iBAQ.R','TPA.R'), '', 'iBAQ [fmol/sample]', 'TPA [fmol/sample]', FALSE)
plotVariability(TPANvsIBAQ, c('iBAQ.R','TPAnorm.R'), '', 'iBAQ [fmol/sample]', 'TPA normalized [fmol/sample]', FALSE)
plotVariability(TPAvsIR, c('iBAQrescaled.R','TPA.R'), '', 'iBAQ rescaled [fmol/sample]', 'TPA [fmol/sample]', FALSE)
plotVariability(TPANvsIR, c('iBAQrescaled.R','TPAnorm.R'), '', 'iBAQ rescaled [fmol/sample]', 'TPA normalized [fmol/sample]', FALSE)
plotVariability(TPANvsTPA, c('TPA.R','TPAnorm.R'), '', 'TPA [fmol/sample]', 'TPA normalized [fmol/sample]', FALSE)
```
...
```{r general-length, fig.height = 7, fig.width = 7, fig.align="center", fig.cap = "Figure S2: Predicted abundances [fmol/sample] Vs protein length by all methods."}
par(mfrow = c(2,2), mar = c(4,4,2,1), cex = 1)
abundanceNames <- c('Abundance.iBAQ.R','Abundance.iBAQrescaled.R','Abundance.TPA.R','Abundance.TPAnorm.R')
plotAbundancesVsLength(SILACdata,abundanceNames)
```
...
```{r accuracy-ups2, fig.height = 7, fig.width = 7, fig.align="center", fig.cap = "Figure S3: Comparison of predicted Vs real abundance values [fmol/sample] from UPS2, according to all methods. Green is a FC < 2, yellow between 2 and 10, and red > 10."}
par(mfrow = c(2,2), mar = c(4,4,2,1), cex = 1)
...
```{r accuracy-cdf, warning = FALSE, fig.height = 5, fig.width = 8, fig.align = "center", fig.cap = "Figure S5: Cumulative distributions of absolute fold changes for both accuracy evaluation metrics: differences of predicted Vs experimental values of UPS2 (left) and differences to median value in ribosomal measurements (right). A fold change of 2 is indicated with a vertical segmented line. Colors represent the methods: iBAQ (red), iBAQ rescaled (green), TPA (blue) and TPA normalized (cyan)."}
...
```{r tot-es, fig.height = 3, fig.width = 10, fig.align = "center", fig.cap = "Figure S6: Total detected protein amounts in all 6 IS, according to all methods."}
...
plotTotalProt(ISdata,'Abundance.TPA.IS')
plotTotalProt(ISdata,'Abundance.TPAnorm.IS')
```

Let's also take a look at the total detected protein amount of each of the 18 samples, colored by the original ES curve used for the calibration (Figure S6):

```{r tot-samples, fig.height = 3, fig.width = 10, fig.align = "center", fig.cap = "Figure S7: Total detected protein amounts in all 18 samples, according to all methods."}
par(mfcol = c(1,4), mar = c(1,2.5,2,1), cex = 1)
plotTotalProt(SILACdata,'Abundance.iBAQ.R..1_')
plotTotalProt(SILACdata,'Abundance.iBAQrescaled.R..1_')
plotTotalProt(SILACdata,'Abundance.TPA.R..1_')
plotTotalProt(SILACdata,'Abundance.TPAnorm.R..1_')
```
...
```{r es-separate, fig.align = "center", fig.cap = "Figure S8: ES curves (log10(abundance [fmol/sample]) in y-axis Vs log10(normalized MS intensity) in x-axis), with (green) and without (red) rescaling. Within each of the 4 orders of magnitude, each symbol corresponds to a different protein."}
...
```{r es-together, fig.align = "center", fig.cap = "Figure S9: ES curves (log10(abundance [fmol/sample]) in y-axis Vs log10(MS intensity) in x-axis), with (green) and without (red) rescaling. Average coefficient of variation (CVm) within each protein is shown in the upper left corner."}
...
```{r precision-all, fig.height = 12, fig.width = 10, fig.align = "center", fig.cap = "Figure S10: Comparison of data variability from iBAQ (1st row), rescaling iBAQ (2nd row), TPA (3rd row) and normalized TPA (4th row). In the variability plots (left and middle columns, log10(abundance [fmol/sample]) both in the x-axis and y-axis), 2 abundance values for a given protein are plotted if they belong to the same replicate (biological or technical, respectively), and green is a FC < 2, yellow between 2 and 10, and red > 10. In the PCA plots (right column), colors refer to technical replicates and shapes to biological replicates."}
...
# 3rd row: TPA
plotVariability(abundanceTPA[,-1],c('.R1.1','.R2.1','.R3.1'),'Biological Variability')
plotVariability(abundanceTPA[,-1],c('_batch1','_batch2','_batch3'),'Technical Variability')
plotPCA(abundanceTPA[,-1],'PCA')
# 4th row: TPA normalized
plotVariability(abundanceTPAN[,-1],c('.R1.1','.R2.1','.R3.1'),'Biological Variability')
plotVariability(abundanceTPAN[,-1],c('_batch1','_batch2','_batch3'),'Technical Variability')
plotPCA(abundanceTPAN[,-1],'PCA')
```
...
```{r precision-tech, fig.width = 4, fig.align = "center", fig.cap = "Figure S11: Fold change cumulative distributions for all methods. A fold change of 2 is indicated with a vertical segmented line. Colors represent the methods: iBAQ (red), iBAQ rescaled (green), TPA (blue) and TPA normalized (cyan)."}
...
```{r precision-abundance, fig.height = 3, fig.width = 10, fig.align = "center", fig.cap = "Figure S12: Fold change Vs abundances for all methods. The detection window of UPS2 and the UPS2 datapoints are highlighted in yellow."}
...
sample3 <- plotFCvsAbundance(SILACdata,ESdata,'Abundance.TPA.R..1_',FALSE)
sample4 <- plotFCvsAbundance(SILACdata,ESdata,'Abundance.TPAnorm.R..1_',FALSE)
```
...
```{r precision-splines, fig.width = 4, fig.align = "center", fig.cap = "Figure S13: Fold change Vs abundances for all methods + smoothing splines. Colors represent the methods: iBAQ (red), iBAQ rescaled (green), TPA (blue) and TPA normalized (cyan). The detection window of UPS2 is highlighted in yellow."}
par(mfrow = c(1,1), mar = c(4,4,1.5,0.5))
