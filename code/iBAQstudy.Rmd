...
For all MS intensitiy values in both iBAQ and SILAC, we need to normalize by the number of theoretical peptides for each protein. We will use sequence length as proxy for number of theoretical peptides:
...
SILACdata <- normalizeIntensities(SILACdata)
```


## 2. Computing absolute abundance of samples
plotTotalProt(iBAQdata,'iBAQ.H.T4h_')
```
...
1. Create a standard curve abundances (fmol/sample) Vs. normalized MS intensities (L fraction in the iBAQ data).
2. Use the standard curve to infer abundances of IS (from the H fraction in the iBAQ values).
3. Use those IS abundances together with the L/H ratios in the SILAC data to compute abundance of samples.
<br>

This method is almost the same as MaxQuant, with the difference that we do not use 
<br>
...
```{r fig.align="center"}
plotTotalProt(iBAQdata,'AbundanceIS.')
```
...
We now use the absolute abundances from the IS (fmol/sample) to infer absolute abundances in each sample of the SILAC data (fmol/sample), by doing:
...
```{r fig.align="center"}
plotTotalProt(SILACdata,'Abundance.R..1_')
```
...
Those fits give us the transformation from light (L) MS intensity of the UPS2 proteins to abundance (fmol/sample). But they don't look the best (consider they are in log), many other curves could almost equally well fit that data. What if instead we assume that all proteins in the IS should add up to the total amount injected, i.e. 6 ug? How would the new ES curves look like?
<br>
...
The alternative: All normalized MS intensities of the H fraction added up together (mass-wise) should be proportional to 6 ug (TPA approach):

1. Compute IS abundances (fmol/sample) in iBAQ: `IS abundance = H intensity * (6 ug)/(sum of all H intensities*MW values)`
2. Use those IS abundances together with the L/H ratios in the SILAC data to compute abundances of samples
<br>

### Step 1: Rescaling data

```{r rescaleIS}
```

```{r}
iBAQdata <- rescaleIS(iBAQdata,'Intensity.H.T4h',6e6)   #6 ug = 6*10^6 pg
```

Let's check the new total detected protein amounts:

```{r fig.align="center"}
plotTotalProt(iBAQdata,'AbundanceRescaledIS.')
```
...
<center>**Figure 5: ES curves (log10(abundance [pg/sample]) in y-axis Vs log10(MS intensity) in x-axis), original (red) Vs rescaled (green)**</center>
<br>
...
<br>
<center>**Figure 6: ES curves (abundance Vs MS intensities of UPS2), original (red) Vs rescaled (green). Average coefficient of variation (CVm) within each protein in the log space is shown in the upper left corner.**</center>
<br>
...
```{r fig.align="center"}
plotTotalProt(SILACdata,'AbundanceRescaled.R..1_')
```
...
<center>**Figure 9: Fold change Vs abundances for all 3 methods. The detection window of UPS2 is highlighted in yellow.**</center>
<br>
...
<center>**Figure 10: Fold change Vs abundances for all 3 methods + smoothing splines (colors corresponding to the colors in figure 16). The detection window of UPS2 is highlighted in yellow.**</center>
<br>
...
1. Compare to actual iBAQ abundances
2. Is the order in ES data always the same?
3. show only technical variability in abundance plots
4. Validation to stoichiometry of ribosomal proteins
5. ES curves relax slope = 1?
