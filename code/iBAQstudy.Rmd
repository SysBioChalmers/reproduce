...
We can go one step further and instead assume all H peaks in SILAC should add up to the average total detection from figure 1 (scaled to 15 ug).

1. Create a standard curve abundances Vs. iBAQ peaks of ES (L fraction in the iBAQ data)
2. Use the standard curve to infer abundances of IS in the iBAQ data (from the H fraction)
3. Add up the total abundance of IS (close to 12 ug)
4. Use this value scaled to 15 ug for computing the IS abundances in SILAC: IS abundance = SILAC.H peak * (total abundance)/(sum of all SILAC.H peaks)
5. Use those IS abundances together with the L/H ratios in the SILAC data to compute abundances of samples

Again, steps 1-3 were already performed in section 2.

#### Step 4: Rescaling data

```{r}
SILACdata <- rescaleIS(SILACdata,'Intensity.H.R',mean_totProt/12*15)  #There are 15 ug of IS in the SILAC data
```

Let's check the total detected protein amounts (all should be the same):

```{r fig.align="center"}
mean_rescaled2 <- plotTotalProt(SILACdata,'AbundanceRescaled2IS.R..1_')
```

<center>**Figure 9: Total detected protein amounts in all 18 IS in SILAC, rescaled**</center>
<br>
...
```{r}
SILACdata <- getSampleAbundance(SILACdata,'','AbundanceRescaled2')
```
...
<center>**Figure 10: Total detected protein amounts in all 18 samples, rescaled by the IS in SILAC**</center>
<br>
...
<center>**Figure 11: Comparison of data variability with (bottom row) and without (top row) rescaling by the IS in SILAC. In the variability plots (left and middle) green is a FC < 2, yellow between 2 and 10, and red > 10. In the PCA plots (right), colors refer to technical replicates and shapes to biological replicates.**</center>
<br>

Variability between technical replicates overall decreases: median fold change between technical replicates drops, and PCA results are less clustered by technical replicate.
