...
As iBAQ values don't add up always to the same totals (see later figure 2), we should also asses the benefits of rescaling all of these values to add up to the injected amounts:
...
<center>**Figure 2: Total detected protein amounts in all 6 IS, according to all 3 methods.**</center>
<br>

Let's also take a look at the total detected protein amount of each of the 18 samples, colored by the original ES curve used for the calibration (figure 2):
...
<center>**Figure 3: Total detected protein amounts in all 18 samples, according to all 3 methods.**</center>
<br>
...
<center>**Figure 4: ES curves (log10(abundance [fmol/sample]) in y-axis Vs log10(MS intensity) in x-axis), with (green) and without (red) rescaling. Within each of the 4 orders of magnitude, each symbol corresponds to a different protein.**</center>
<br>
...
<br>
<center>**Figure 5: ES curves (log10(abundance [fmol/sample]) in y-axis Vs log10(MS intensity) in x-axis), with (green) and without (red) rescaling. Average coefficient of variation (CVm) within each protein is shown in the upper left corner.**</center>
<br>
...
# Top row: MaxQuant
abundance <- SILACdata[,grep('Abundance.iBAQ.R',names(SILACdata))]
...
abundance <- SILACdata[,grep('Abundance.iBAQrescaled.R',names(SILACdata))]
plotVariability(abundance,c('.R1.1','.R2.1','.R3.1'),'Biological Variability')
plotVariability(abundance,c('_batch1','_batch2','_batch3'),'Technical Variability')
plotPCA(abundance,'PCA')
