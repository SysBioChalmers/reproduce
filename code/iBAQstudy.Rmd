...
Let's take a look at the ES real abundance values Vs the normalized intensity data from method 4, and compare the "linear model" mentioned in section \ref{method-4-normalized-tpa} (method 4) to a linear fit to the data (method 1). For that we need:

* A function for plotting a linear fit.
* A function for plotting the external standard data + fits.
...
The blue fits give us the transformation from light (L) MS intensity of the UPS2 proteins to abundance (fmol/sample) with iBAQ. But they don't look the best (considering they are in log10 space), many other curves (as the yellow fits) can almost equally well fit that data. Let's see everything in the same plot:
