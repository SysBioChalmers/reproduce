...
iBAQdata  <- rescaleData(iBAQdata,'Abundance.MaxQuant.IS.','MQrescaled.IS.',6e6) #6 ug = 6*10^6 pg
SILACdata <- getSampleAbundance(SILACdata,iBAQdata,'Abundance.MQrescaled')
```
...
iBAQdata  <- rescaleData(iBAQdata,'Intensity.H.T4h_','MSrescaled.IS.',6e6) #6 ug = 6*10^6 pg
SILACdata <- getSampleAbundance(SILACdata,iBAQdata,'Abundance.MSrescaled')
```
...
plotTotalProt(iBAQdata,'Abundance.MQrescaled.IS')
plotTotalProt(iBAQdata,'Abundance.MSrescaled.IS')
```
...
<br>
<center>**Figure 4: ES curves (abundance Vs MS intensities of UPS2), from method 2 (red) Vs method 4 (green). Average coefficient of variation (CVm) within each protein in the log space is shown in the upper left corner.**</center>
<br>
...
* make all axes the same levels
* changelog10 axes to log2 axes
