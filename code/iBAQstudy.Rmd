...
2. Use the standard curve to infer abundances of IS in the iBAQ data (from the H fraction)
...
1. Create a standard curve abundances Vs. iBAQ peaks of ES (L fraction in the iBAQ data) 
2. Use the standard curve to infer abundances of IS in the iBAQ data (from the H fraction)
3. Add up the total abundance of IS (close to 12 ug)
4. Use this value for computing the IS abundances in iBAQ: IS abundance = iBAQ.H peak * (total abundance)/(sum of all iBAQ.h peaks)
5. Use those IS abundances together with the L/H ratios in the SILAC data to compute abundances of samples

Note that steps 1-3 were already performed in section 2.
...
We can go one step further and instead discard all iBAQ scores, i.e. construct ES curves from the UPS2 raw MS peaks, compute the average detected protein amount for IS, and assume all H peaks in SILAC should add up to that (as in the previous approach).

1. Create a standard curve abundances Vs. raw peaks of ES (L fraction in the iBAQ data)
2. Use the standard curve to infer abundances of IS in the iBAQ data (from the H fraction)
3. Add up the total abundance of IS (close to 12 ug)
4. Use this value for computing the IS abundances in iBAQ: IS abundance = raw.H peak * (total abundance)/(sum of all raw.H peaks)
5. Use those IS abundances together with the L/H ratios in the SILAC data to compute abundances of samples


#### Step 1: Getting ES curves

Let's start by checking out how are the ES curves if we use raw intensity peaks instead of the iBAQ score:
...
The fits are actually better than in figure 3, suggesting that, at least for this dataset, protein abundance correlates better to raw MS peaks than iBAQ scores. We will then proceed with only raw MS peaks in the analysis.
<br>
...
<center>**Figure 11: Total detected protein amounts in all 18 IS in iBAQ, rescaled**</center>
<br>
...
<center>**Figure 12: ES curves for all 6 measurements of UPS2 based on raw peaks, original (red) Vs rescaled (green)**</center>
<br>
...
<br>
<center>**Figure 13: ES curves for all 6 measurements of UPS2, original (red) Vs rescaled (green)**</center>
<br>

Again, comparing these fits to figure 6, we see how the variability for a single protein measurement is higher in iBAQ scores than in simple raw MS peaks.

#### Step 5: Getting sample abundances from IS

Now we compute the SILAC abundances based on these rescaled values:
...
Let's look at the variability with all 3 methods:
...
plotVariability(abundance,c('_Batch1','_Batch2','_Batch3'),'Technical Variability')
plotPCA(abundance,'PCA')
# Bottom row: Rescaling to SILAC vakues
...
In conclusion, we can skip entirely the iBAQ data and just assume a proportionality between SILAC.H peaks and abundance. This achieves more reproducible results than the iBAQ alternative. The only way the iBAQ dat is used here is for estimating the total detection fraction of protein, but this could be run as any other SILAC run.
...
5. Are these findings supported by your datasets? In particular, do you also observe what I show in figure 9 VS figure 3?
6. How relevant are these findings? Old news or actually something interesting? Most times I've only seen a single ES curve and no technical replicates...
