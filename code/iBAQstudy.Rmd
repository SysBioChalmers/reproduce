...
* Rescaling values to a fix total protein abundance based on iBAQ peaks, to asses the quality of the ES curves (section 3)
* Rescaling values to a fix total protein abundance based on raw MS peaks, to asses the usefulness of iBAQ peaks (section 4).
...
* Raw MS peaks overall have less variability and exhibit better correlation to protein abundance than iBAQ peaks, hence are better suited for estimating protein levels.
<br>
...
1. Create a standard curve abundances Vs. iBAQ peaks of ES (L fraction in the iBAQ data)
2. Use the standard curve to infer abundances of IS in the iBAQ data (from the H fraction of the iBAQ peaks)
...
## 3. Rescaling abundances to iBAQ peaks

### Computing strategy

The alternative: All H peaks in iBAQ added up together should result in the average total detection from figure 1.

1. Create a standard curve abundances Vs. iBAQ peaks of ES (L fraction in the iBAQ data)
2. Use the standard curve to infer abundances of IS in the iBAQ data (from the H fraction of the iBAQ peaks)
3. Compute the average total detected protein amount of IS
4. Use this value for rescaling the IS abundances in iBAQ: `IS abundance = iBAQ.H peak * (average total IS abundance)/(sum of all iBAQ.H peaks)`
...
`abundance = m*Hpeak`, where `m = (average total IS abundance)/(sum of all Hpeaks)`
...
scaling <- mean_totProt/colSums(iBAQdata[,Hpeaks], na.rm = TRUE)
plotAllES(ESdata,'iBAQ.L.T4h_',scaling,FALSE)
```

<center>**Figure 5: ES curves (abundance Vs iBAQ peaks of UPS2), original (red) Vs rescaled (green)**</center>
<br>
...
<br>
<center>**Figure 6: ES curves (abundance Vs iBAQ peaks of UPS2), original (red) Vs rescaled (green). Average coefficient of variation (CVm) within each protein in the log space is shown in the upper left corner.**</center>
<br>
...
<center>**Figure 8: Comparison of data variability with (bottom row) and without (top row) rescaling. In the variability plots (left and middle columns) green is a FC < 2, yellow between 2 and 10, and red > 10. In the PCA plots (right column), colors refer to technical replicates and shapes to biological replicates.**</center>
<br>
...
In conclusion, by assuming that the iBAQ data should always adds up to a given protein amount, we can reproduce almost exactly the ES curves, and achieve more consistent results across samples. In this approach, the UPS2 data is only used to get an estimate of the detected total protein amount of the IS by the iBAQ peaks.

One thought still remains: Why use iBAQ peaks to begin with? Can we achieve similar quality of data if we only look at the raw MS peaks?
<br>

## 4. Rescaling abundances to raw MS peaks

### Computing strategy

We can go one step further and instead discard all iBAQ peaks, i.e. construct ES curves from the UPS2 raw MS peaks, compute the average detected protein amount for IS, and assume all H peaks in SILAC should add up to that (as in the previous approach).

1. Create a standard curve abundances Vs. raw peaks of ES (L fraction in the iBAQ data)
2. Use the standard curve to infer abundances of IS in the iBAQ data (from the H fraction of the raw MS peaks)
3. Compute the average total detected protein amount of IS
4. Use this value for computing the IS abundances in iBAQ: `IS abundance = raw.H peak * (average total IS abundance)/(sum of all raw.H peaks)`
5. Use those IS abundances together with the L/H ratios in the SILAC data to compute abundances of samples
<br>

### Step 1: Getting ES curves

Let's start by checking out how are the ES curves if we use raw intensity peaks instead of the iBAQ peaks:

```{r fig.align="center"}
plotAllES(ESdata,'Intensity.L.T4h_',0,FALSE)
```

<center>**Figure 9: ES curves for all 6 measurements of UPS2, based on raw L peaks**</center>
<br>

The fits are actually better than in figure 3, suggesting that, at least for this dataset, protein abundances correlate better to raw MS peaks than iBAQ peaks. We will then proceed with only raw MS peaks in this section.
<br>

### Steps 2-3: Getting IS abundances from ES

We repeat the process of getting IS abundances but now using raw peaks instead of iBAQ peaks:
...
```{r fig.align="center"}
mean_totProt <- plotTotalProt(iBAQdata,'AbundanceRawIS.')
```

<center>**Figure 10: Total detected protein amounts in all 6 IS, based on raw H peaks**</center>
<br>
...
```{r fig.align="center"}
mean_rescaled2 <- plotTotalProt(iBAQdata,'AbundanceRawRescaledIS.')
```

<center>**Figure 11: Total detected protein amounts in all 6 IS in iBAQ, rescaled based on raw H peaks**</center>
<br>

Let's now check that the new fits are not too far away to the ones in figure 9:

```{r fig.align="center"}
Hpeaks <- grep('Intensity.H.T4h',names(iBAQdata))   #All 6 absolute raw peaks from the IS (H fraction)
scaling <- mean_totProt/colSums(iBAQdata[,Hpeaks], na.rm = TRUE)
plotAllES(ESdata,'Intensity.L.T4h_',scaling,FALSE)
```

<center>**Figure 12: ES curves (abundance Vs raw H peaks of UPS2), original (red) Vs rescaled (green)**</center>
<br>
...
<br>
<center>**Figure 13: ES curves (abundance Vs raw H peaks of UPS2), original (red) Vs rescaled (green). Average coefficient of variation (CVm) within each protein in the log space is shown in the upper left corner.**</center>
<br>

Comparing these fits to figure 6, we see how the variability for a single protein measurement is higher in iBAQ peaks than in simple raw MS peaks (CVm goes down).
<br>
...
```{r}
SILACdata <- getSampleAbundance(SILACdata,iBAQdata,'AbundanceRawRescaled')
```

```{r fig.align="center"}
mean_sampleRescaled2 <- plotTotalProt(SILACdata,'AbundanceRawRescaled.R..1_')
```
...
plotVariability(abundance,c('_Batch1','_Batch2','_Batch3'),'Technical Variability')
plotPCA(abundance,'PCA')
# Middle row: Rescaling to iBAQ peaks
abundance <- SILACdata[,grep('AbundanceRescaled.R',names(SILACdata))]
plotVariability(abundance,c('.R1.1','.R2.1','.R3.1'),'Biological Variability')
plotVariability(abundance,c('_Batch1','_Batch2','_Batch3'),'Technical Variability')
plotPCA(abundance,'PCA')
# Bottom row: Rescaling to raw MS peaks
abundance <- SILACdata[,grep('AbundanceRawRescaled.R',names(SILACdata))]
plotVariability(abundance,c('.R1.1','.R2.1','.R3.1'),'Biological Variability')
plotVariability(abundance,c('_Batch1','_Batch2','_Batch3'),'Technical Variability')
plotPCA(abundance,'PCA')
```

<center>**Figure 15: Comparison of data variability without rescaling (top row), rescaling to iBAQ peaks (middle row) and rescaling to raw peaks (bottom row). In the variability plots (left and middle columns) green is a FC < 2, yellow between 2 and 10, and red > 10. In the PCA plots (right column), colors refer to technical replicates and shapes to biological replicates.**</center>
<br>

Note how we progressively increase the quality of the data: median fold change between technical replicates slowly drops while maintaining the same difference between biological replicates, and the first component in the PCA analysis comprises less and less variability.

In conclusion, we can skip entirely the iBAQ data and just assume a proportionality between raw MS peaks and abundance. This achieves even more reproducible results than the iBAQ alternative (section 3). The only way the iBAQ data is used here is for estimating the total detection fraction of protein, but this could be run as any other SILAC sample, as the iBAQ peaks are never used.
...
4. Is it possible to assume that raw L or H peaks are proportional to abundance? If not, how are iBAQ peaks different?
5. How relevant are these findings? Old news or actually something interesting? Most times I've only seen a single ES curve and no technical replicates. Also not sure if someone has shown how the variability of iBAQ peaks is > variability of raw peaks.
