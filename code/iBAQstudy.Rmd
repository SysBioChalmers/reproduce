...
As iBAQ values don't add up always to the same totals (see later figure 3), we should also asses the benefits of rescaling all of these values to add up to the injected amounts:
...
plotESdata(ESdata,'iBAQ')
plotESdata(ESdata,'iBAQrescaled')
plotESdata(ESdata,'MSrescaled')
```
...
We see that all predictions are very similar; by using ES curves (methods 1-2) we don't gain significant prediction power than if we just rescale the data (method 3).
...
abundanceIBAQ <- SILACdata[,grep('Abundance.iBAQ.R',names(SILACdata))]
abundanceIR   <- SILACdata[,grep('Abundance.iBAQrescaled.R',names(SILACdata))]
abundanceMSR  <- SILACdata[,grep('Abundance.MSrescaled.R',names(SILACdata))]
IRvsIBAQ  <- cbind(abundanceIBAQ,abundanceIR)
MSRvsIBAQ <- cbind(abundanceIBAQ,abundanceMSR)
...
<center>**Figure 3: Total detected protein amounts in all 6 IS, according to all 3 methods.**</center>
<br>

Let's also take a look at the total detected protein amount of each of the 18 samples, colored by the original ES curve used for the calibration (figure 3):
...
<center>**Figure 4: Total detected protein amounts in all 18 samples, according to all 3 methods.**</center>
<br>
...
<center>**Figure 5: ES curves (log10(abundance [fmol/sample]) in y-axis Vs log10(MS intensity) in x-axis), with (green) and without (red) rescaling. Within each of the 4 orders of magnitude, each symbol corresponds to a different protein.**</center>
<br>
...
<br>
<center>**Figure 6: ES curves (log10(abundance [fmol/sample]) in y-axis Vs log10(MS intensity) in x-axis), with (green) and without (red) rescaling. Average coefficient of variation (CVm) within each protein is shown in the upper left corner.**</center>
<br>
...
# Top row: MaxQuant
plotVariability(abundanceIBAQ,c('.R1.1','.R2.1','.R3.1'),'Biological Variability')
plotVariability(abundanceIBAQ,c('_batch1','_batch2','_batch3'),'Technical Variability')
plotPCA(abundanceIBAQ,'PCA')
# Middle row: Interpolating with ES curve
plotVariability(abundanceIR,c('.R1.1','.R2.1','.R3.1'),'Biological Variability')
plotVariability(abundanceIR,c('_batch1','_batch2','_batch3'),'Technical Variability')
plotPCA(abundanceIR,'PCA')
# Bottom row: Rescaling
plotVariability(abundanceMSR,c('.R1.1','.R2.1','.R3.1'),'Biological Variability')
plotVariability(abundanceMSR,c('_batch1','_batch2','_batch3'),'Technical Variability')
plotPCA(abundanceMSR,'PCA')
```

<center>**Figure 7: Comparison of data variability from iBAQ (top row), rescaling iBAQ (middle row) and rescaling the normalized MS intensities (bottom row). In the variability plots (left and middle columns, log10(abundance [fmol/sample]) both in the x-axis and y-axis), 2 abundance values for a given protein are plotted if they belong to the same replicate (biological or technical, respectively), and green is a FC < 2, yellow between 2 and 10, and red > 10. In the PCA plots (right column), colors refer to technical replicates and shapes to biological replicates.**</center>
<br>
...
Let's look further into the reduction of variability by plotting all possible fold changes between technical replicates. For that we define the following plotting function that will plot 
...
FCiBAQ <- plotCumulativeDistrib(abundanceIBAQ,c('_batch1','_batch2','_batch3'),'red',TRUE)
FCiBAQres <- plotCumulativeDistrib(abundanceIR,c('_batch1','_batch2','_batch3'),'green',FALSE)
FCMSres <- plotCumulativeDistrib(abundanceMSR,c('_batch1','_batch2','_batch3'),'blue',FALSE)
```

<center>**Figure 8: Fold change cumulative distributions for all 3 methods. A fold change of 2 is indicated with a vertical segmented line.**</center>
<br>

These 3 distributions are significantly different between them:

```{r}
htest <- ks.test(FCiBAQ, FCiBAQres, alternative = 'less')
print(htest$p.value)
htest <- ks.test(FCiBAQres, FCMSres, alternative = 'less')
print(htest$p.value)
```
...
<center>**Figure 9: Fold change Vs abundances for all 3 methods. The detection window of UPS2 and the UPS2 datapoints are highlighted in yellow.**</center>
<br>
...
<center>**Figure 10: Fold change Vs abundances for all 3 methods + smoothing splines (colors corresponding to the colors in figure 6). The detection window of UPS2 is highlighted in yellow.**</center>
<br>
