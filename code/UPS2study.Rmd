...
```{r fig.align="center"}
mean_totProt <- protPlot(iBAQdata,'Abundance.')
```
...
```{r fig.align="center"}
mean_sample <- protPlot(SILACdata,'Abundance.R..1_')
```
...
```{r fig.align="center"}
mean_rescaled <- protPlot(iBAQdata,'AbundanceRescaled.')
```
...
ESplots(ESdata,mean_totProt/colSums(iBAQdata[,Hpeaks], na.rm = TRUE),FALSE)
```
...
```{r fig.align="center"}
ESplots(ESdata,mean_totProt/colSums(iBAQdata[,Hpeaks], na.rm = TRUE),TRUE)
```
...
```{r fig.align="center"}
mean_sampleRescaled <- protPlot(SILACdata,'AbundanceRescaled.R..1_')
```
...
2. Is rescaling to the average total amount the best strategy? It requires then for any proteomic study to run several ES curves (in order to get an idea of the variability of the MS machine). Maybe the detection percentage could be estimated in another way? I think this might be the "purest" way of estimating it, as it relies only on the MS data and not in any external database (PaxDB, uniprot, etc)
3. Are ES curves always constructed with a slope = 1? Also not sure if this was a given in the field or people actually fit both a and b in y=ax+b
4. How relevant are these findings? Old news in the field or actually something interesting? Most times I've only seen a single ES curve...