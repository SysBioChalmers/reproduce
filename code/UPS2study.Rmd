...
```{r}
ESplots(ESdata,1,1,FALSE,FALSE)
```
...
ESplots(ESdata,12*1e6/colSums(iBAQdata[,Hpeaks], na.rm = TRUE),1,TRUE,FALSE)
```

The fits are almost the same, validating our approach. Let's see everything in the same plot:

```{r}
ESplots(ESdata,12*1e6/colSums(iBAQdata[,Hpeaks], na.rm = TRUE),1,TRUE,TRUE)
```
